...
void carARCH(vuelo vec[],int MAX)
{
    ...
    if(c=='S')
    {
        ...
        fprintf(Aw2,"CODIGO;DIA;VUELO;COSTE;PASAJEROS\n");
    }
    ...
    for(i=0;i<=MAX;i++)
    {
        ...
        if(c=='S')
        {
           fprintf(Aw2,"%s;%d;%d;%f;%d\n",vec[i].codAerolinea,vec[i].DIA,vec[i].nVuelo,vec[i].costPasaje,vec[i].cantPasajeros);
        }
    }
